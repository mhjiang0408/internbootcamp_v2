...
推荐使用 [uv](https://github.com/astral-sh/uv) 来准备可重复的 Python 环境。完成 `uv` 安装后，按照下列步骤初始化项目依赖：

```bash
# 1) 创建虚拟环境（默认会使用项目声明的 Python 版本）
uv venv .venv
...
`pyproject.toml` 中已经声明了评测与工具服务器所需的三方库（`openai`, `httpx`, `aiohttp`, `fastapi`, `tqdm`, `pandas`, `transformers` 等），因此只需执行上述流程即可获得完整的运行环境。
